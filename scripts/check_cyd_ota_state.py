"""
One-shot: reads the current retained printer MQTT snapshot for the fields
relevant to a stuck/failed CYD OTA attempt - what version it's actually
running right now, whether it thinks an update is in progress, WiFi/Bambu
connection state, and free heap (a failed OTA can be a heap/OOM issue).
...
def main():
    ...
    client = mqtt.Client(client_id="check-cyd-ota-state", protocol=mqtt.MQTTv311)
